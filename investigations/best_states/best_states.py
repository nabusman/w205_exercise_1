...
hospital_metrics = effective_careDF.filter(effective_careDF["score"].rlike("\d+")).select(effective_careDF["state"], effective_careDF["score"].cast("integer").alias("score_int"), effective_careDF["sample"].cast("integer").alias("sample_int"))
hospital_metrics = hospital_metrics.groupBy("state").agg(func.avg(hospital_metrics["score_int"]).alias("average_score"), 
...
hospital_metrics.orderBy(hospital_metrics["effectiveness"].desc()).limit(10).show()